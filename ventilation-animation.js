/**
 * Ventilation Duct Animation Module
 * Стилизовано под дизайн-систему ВентПром
 */
...
(function() {
    ...
    // ==========================================
    const CONFIG = {
        containerId: 'ventilation-animation-container',
        colors: {
            bg: 0x0D0D0D,
            fill: 0x1A1A1A,
            line: 0xE5E5E5,
    ...
    const layers = [
        { color: 0xB0B0B0, size: 0.1, maxLife: 250, speed: 1.2, frequency: 2 },
        { color: 0xFFFFFF, size: 0.3, maxLife: 200, speed: 1.1, frequency: 3 },
        { color: 0xB0B0B0, size: 0.5, maxLife: 150, speed: 0.9, frequency: 4 },
        { color: 0x757575, size: 0.7, maxLife: 100, speed: 0.8, frequency: 5 }
    ];
    ...
    function createDuctSystem() {
        ...
        const linesBend = new THREE.LineSegments(edgesBend, new THREE.LineBasicMaterial({ 
    ...
    function createInternalComponents() {
        ...
        const b1Mesh = new THREE.Mesh(bladeGeo, createGhostMaterial(0.5));
        ...
        const b2Mesh = new THREE.Mesh(bladeGeo, createGhostMaterial(0.5));
        ...
        const shaftMesh = new THREE.Mesh(shaftGeo, createGhostMaterial(0.4));
        ...
        const hubMesh = new THREE.Mesh(hubGeo, createGhostMaterial(0.4));
        ...
        const ringGeo = new THREE.TorusGeometry(rotorRadius, 0.02, 8, 32);
        const ringMesh = new THREE.Mesh(ringGeo, createGhostMaterial(0.3));
    ...
    function createEnvironment() {
        const grid = new THREE.GridHelper(30, 60, CONFIG.colors.lineMuted, CONFIG.colors.bg);
    ...
    function initParticleSystem() {
        ...
        gradient.addColorStop(0.3, 'rgba(229,229,229,0.6)');
        gradient.addColorStop(1, 'rgba(229,229,229,0)');
